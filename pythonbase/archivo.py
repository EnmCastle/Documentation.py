...
c = open(
    "hola.txt", "w"
)  # En nuestra carpeta vamos a ver que se creo un hola.txt + nuestro texto de write
...
if os.path.exists(
    "Archivo.txt"
):  # Esta linea es para no retornar errores y esta preguntando si esxiste usando condiciones
    os.remove(
        "Archivo.txt"
    )  # Des esta forma sera eliminado pero en caso de que no exista nos retornará un error
else:
    c.write(
        "Arquivo.txt"
    )  # Aqui decidimos crear este arvhivo y ahora esta en nuestra carpeta
...
if os.path.exists(
    "Carpetadeejemplo"
):  # El mismo ejemplo de comprovación de existencia de nuestra carpeta o archivo
